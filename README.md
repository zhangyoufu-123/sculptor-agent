...
**一个会先读懂你、再陪你写好的深度协作写作 Agent（v0.68）。**
...
- **改迹调制（v0.68）**：把作者每次亲手修改（原文→改后→意图）当作偏好标注，学习十二维
  外层调制器权重；写作每节并行生成 n 个候选，十二维评分选优，并给出"为什么选它"的得分分解
  与人话理由。几十次修改即可学到稳定的作者方向（学习曲线实验，n≈15–20 进入平台）。
- **个人回避库 + 可解释层（v0.68）**：聚合"作者亲手删掉的词"作为第 12 维特征；上下文窗口
  编辑对让"修改即标注"落到实处。
- **姿态层诊断前置（v0.67）**：把"假思考"细读判据（金句排比/路标转折/点题顿悟）前置到候选
  排序，软性加权，不拒绝生成。
- **L3 作者写作清单（v0.66）**：主张/论证/读者/红线/触发五问自动归纳，深层风格读取结构化并
  直接参与评分。
- **神经风格编码 + 外层调制器（v0.64–v0.65）**：可选 embedding 稠密原型、知识库 BM25+语义
  混合检索、偏好对权重学习与增量在线更新——从"风格签名"迈向"作者模型"。
- **统一 Token 对比解码 V1（v0.62）**：本地字符级 n-gram 个人模型 + 并行候选评分选优，
  得分分解可追溯。更早的实时大纲（v0.57）、联网查验（v0.58）、外溢优先（v0.59）、全流程
  互操作（v0.60）、批注闭环（v0.61）见下文与 [CHANGELOG.md](CHANGELOG.md)。
...
3. **人物风格肖像**：把知识库、旧作、修改记录侧写成一幅"写作人格画像"，随时可查（`sculptor persona`）。
