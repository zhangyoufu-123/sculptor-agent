...
1. **权重学习**：`agent/src/modulator.js` 把 edits.jsonl 的（原文，改后）当偏好对，
   用 pairwise hinge + SGD 学十一维权重 w_i——签名正式升级为可学习模型（见 MODULATOR.md）；
2. **十一维特征入评分**：表层/话语/立场红线/知识/缺陷/阻抗/风格向量方向 + 个人 n-gram +
...
7. **降级保底**：编辑对不足时回退经验默认权重，不阻塞写作。

**下一步**：
1. **姿态层细读判据入评分**：把 LLM 六层细读的结构化输出（声音/过渡/修辞判据）加入 f_i；
2. **细读判据精读化（可选）**：LLM 六层细读输出作为可选精读特征（当前为确定性判据前置）；
