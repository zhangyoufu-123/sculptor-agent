...
**边界（诚实界定）**：① 文本 API 无法提供完整词表 logits，V1/V1.5 只能做候选/句段级对比，逐 token 重排需要 logprobs（V2）或本地推理（V3）；② $p_{\mathrm{personal}}$ 仍是 n-gram 代理模型，真实条件分布依赖轻量 LoRA（V3）；③ 调制器权重已由作者偏好对学习（即对 $\beta$/$\lambda$/$R$ 的自动标定），但消融标定已完成（学习权重 90% > 默认权重 70%，见 MODULATOR-ABLATION.md），第三方盲评尚未完成，候选数自适应列为后续工作。本文不宣称"逐 token 风格解码已实现"，只交付已实现的签名/注入层、可学习调制器与可复现的评分器设计。
