...
**未来工作**：① **统一解码框架 V1 已落地（v0.62，候选对比 + 个人 n-gram 模型）**，下一步做五路权重消融标定与候选数自适应，进而 V2 logprobs（显式 $\beta_1$）、V3 本地轻量作者模型（LoRA）全词表融合；② 扩大多作者 × 多篇跨文体语料，完成作者识别/续写选择的显著性检验；③ 执行澄清协议与外溢优先的消融对照实验（4.4 设计）；④ 跨语言验证与公开部署；⑤ 批注数据反哺——把用户批注积累为错误模式库与风格档案（v0.61 已打通"批注 → AI 修改"闭环）。
