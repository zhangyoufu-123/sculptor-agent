...
- 注入不进入状态文件，不污染上下文；用户随时可用
  `sculptor knowledge` 查看/搜索/增删自己的知识。

## CLI 管理

```bash
sculptor knowledge                  # 列表（标题/类型/使用次数/收录时间）
sculptor knowledge search 地坛      # 按主题检索（score 排序）
sculptor knowledge view 《我与地坛》 # 查看单条（含笔记）
sculptor knowledge add 《X》 --type book --author 作者 --note 备注
sculptor knowledge remove 《X》     # 移除（可恢复：从 git/备份恢复该 md）
```
