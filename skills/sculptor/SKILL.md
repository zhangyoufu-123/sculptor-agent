...
`sculptor style --signals` 可回看"每一轮我读到了什么"。
...
`sculptor rag ingest-assets <results.json>` 回灌），结果中的《书名》自动入知识库，后续直接推荐。
...
用户随时可查：`sculptor persona`；重新生成：`sculptor persona --refresh`。样本不足时确定性兜底，不阻塞。
...
续写/系列文时读取注入，保证前后不打架。`sculptor bible list|view|save|distill` 可管理。
...
- `sculptor knowledge export [--to file]` / `import <file>`：个人知识库随人走（按标题去重合并，
  含提问记录）；导出会提示隐私——知识库含个人阅读/经历，勿传公开仓库。
- `sculptor emotion`：把成稿按节量化情绪曲线（平静/喜悦/哀伤/愤怒/张力，0-1 强度 + 主导情绪），
  供节奏检查（叙事弧/情绪曲线思路）。
- `sculptor curve`（v0.41）：三线节奏曲线——每节 张力/信息密度/情绪强度/节奏变化 四条 0-100
  曲线，落盘 `vault/curve.md`，像看心电图一样发现"哪里太平、哪里该提情绪"，供二次编辑。
- `sculptor consistency`（v0.41）：伏笔回收校验——小说/推理每写一节自动记账伏笔（LLM 提炼、
...
**澄清深度阶梯（按序挖透，主次分明，v0.43）**：主题 → **表达欲望与行文思路**
