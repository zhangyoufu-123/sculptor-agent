...
CLI="$SKILL_DIR/scripts/sculptor.mjs"
DRY_RUN=0
TARGETS=()
...
install_into() {
  ...
  cp -R "$SKILL_DIR" "$dest"
  chmod +x "$dest/scripts/sculptor.mjs" "$dest/scripts/install.sh" "$dest/hooks/sculptor-hook.sh" 2>/dev/null || true
  printf '[%s] 已安装 → %s\n' "$host" "$dest"
}

detect_codex() {
  local base="${CODEX_HOME:-$HOME/.codex}"
  [ -d "$base" ] && install_into "$base/skills/sculptor" "codex"
}

detect_claude() {
  install_into "$HOME/.claude/skills/sculptor" "claude"
}

detect_opencode() {
  for base in "$HOME/.config/opencode/skills" "$HOME/.opencode/skills"; do
    [ -d "$(dirname "$base")" ] && { install_into "$base/sculptor" "opencode"; return; }
  done
  install_into "$HOME/.config/opencode/skills/sculptor" "opencode"
}

wire_codex_hooks() {
  local config="${CODEX_HOME:-$HOME/.codex}/config.toml"
  local hook_path="${CODEX_HOME:-$HOME/.codex}/skills/sculptor/hooks/sculptor-hook.sh"
  local hermes=0
  for arg in "$@"; do
    [ "$arg" = "--hermes" ] && hermes=1
  done
  ...
  printf '生效说明: 重启 Codex（或新开会话）后生效；事件载荷由 sculptor-hook.sh 容错处理。\n'
}

init_project() {
  if [ "$DRY_RUN" -eq 1 ]; then
    printf '[dry-run] init → %s/.sculptor\n' "${1:-.}"
    return
  fi
  node "$CLI" init "${1:-.}"
}
