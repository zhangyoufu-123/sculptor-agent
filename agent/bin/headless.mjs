#!/usr/bin/env node
// Sculptor 无头引擎桥（headless bridge）：供 FastAPI / 任意后端以子进程方式调用。
// 输入：stdin 一段 JSON —— {"message":"用户输入","workspace":"/abs/会话目录"}
// 凭据：环境变量 BYOK —— SCULPTOR_LLM_API_KEY / SCULPTOR_LLM_BASE_URL / SCULPTOR_LLM_MODEL
// 输出：stdout 一段 JSON —— agentStep 的结果 + 轻量状态摘要（或 {"error":"..."}）。
// 离线：SCULPTOR_MOCK_LLM=1 时用内置 mock（便于后端冒烟测试）。
