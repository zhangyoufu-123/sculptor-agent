  ...
  if (d.stage === 'quality') {
    ...
    try {
      if (cfg.roundtrip === false) {
        state.quality.roundtrip = { skipped: true, reason: '已禁用（STYLOTRACE_ROUNDTRIP=0）' };
      } else if (draftText.replace(/\s/g, '').length > 3000) {
        state.quality.roundtrip = { skipped: true, reason: '超过 3000 字单批上限，可手动运行 stylotrace roundtrip' };
      } else {
