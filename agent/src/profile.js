...
/** 导出风格 bundle（默认写入 SCULPTOR_HOME 或当前目录 sculptor-profile.json）。 */
export function exportProfile(workspace, outFile = '') {
  ...
  const dest = outFile
    ...
    : process.env.SCULPTOR_HOME
      ? path.join(process.env.SCULPTOR_HOME, 'style-profile.json')
...
export function profileStatus(workspace) {
  ...
  return {
    ...
    globalPath: process.env.SCULPTOR_HOME
      ? path.join(process.env.SCULPTOR_HOME, 'style-profile.json')
      : '',
  };
}
